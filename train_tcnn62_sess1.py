...
from tcnn62 import tCNN
from utils.ckpt_util import save_checkpoint
import logging
from cnn_data_loader_62.data_loaders import *
...
def main(logger, result_file, sub):
    ...
    optimizer = torch.optim.Adam(model.parameters(), 1e-3, weight_decay=0.01)  # 对参数进行正则化weight_decay, 防止过拟合
...
if __name__ == '__main__':
    ...
    for sess_index, sess in enumerate(sess_list):
        ...
        print(sess)
        acc_list = []
        result_file = open(os.path.join('./result_dir', 'sess1_62.txt'), 'w')
